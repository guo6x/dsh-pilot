...
| `pilot_snapshot` | 把当前页面读成文本：标题、URL、可见文本（8k 字符）、链接、**编号元素列表（refs）**，以及相对上次快照的**变化摘要** |
...
| `pilot_wait_for` | 等待页面文本、可见元素或 URL 条件满足，替代盲等 |
| `pilot_assert` | 立即验证页面文本、可见元素或 URL 条件是否满足 |
...
可拖拽驾驶舱浮窗：实时截图（2 秒刷新）、当前 URL + 标题、启动/关闭按钮、地址栏、最近操作日志。多个会话同时开车时，可手动查看任意一个，或选择**最近活跃**继续自动跟随 agent。不想让它开了，一键关掉；想自己上手，随时接管。
...
- **面板默认显示最近活跃会话的浏览器**：每个会话仍是独立实例；会话多时可手动锁定任意一个，选择**最近活跃**即可恢复自动跟随。
