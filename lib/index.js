...
var LOOPBACKS = /* @__PURE__ */ new Set(["127.0.0.1", "::1", "::ffff:127.0.0.1"]);
var EDGE_CANDIDATES = [
...
var MAX_FILL_FIELDS = 20;
var MAX_UPLOAD_FILES = 10;
var NAV_TIMEOUT_MS = 2e4;
var POOL_CAP = 8;
var PROFILE_PREFIX = "dsh-pilot-";
var PROFILE_DIR_PATTERN = /^dsh-pilot-[A-Za-z0-9]{6}$/;
var PROFILE_REMOVE_ATTEMPTS = 4;
var PROFILE_REMOVE_BACKOFF_MS = 200;
var STALE_PROFILE_AGE_MS = 60 * 60 * 1e3;
var PROFILE_BACKLOG_ATTEMPTS = 3;
var liveProfileDirs = /* @__PURE__ */ new Set();
var profileCleanupBacklog = /* @__PURE__ */ new Map();
var profileSweepStarted = false;
...
async function gcStaleProfiles() {
...
var Cdp = class {
  ...
  once(method, timeoutMs = 15e3) {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        const list2 = this.events.get(method) ?? [];
        const at = list2.indexOf(listener);
...
var Pilot = class {
  ...
  findEdge() {
    ...
    if (found === void 0) throw new Error("pilot: no Edge/Chrome executable found; install one or set edgePath");
    return found;
  }
  ...
  async waitForVersion(port) {
    ...
    throw new Error(foreign ? `pilot: port ${port} is answering for another browser session; retry to pick a free port` : "pilot: browser did not expose the debugging endpoint in time");
  }
  ...
  async navigate(url) {
    ...
    await Promise.race([this.cdp.once("Page.loadEventFired", NAV_TIMEOUT_MS).catch(() => {
  ...
  async waitForLoad(timeoutMs = 3e3) {
  ...
  async wait(ms) {
    const clamped = Math.max(0, Math.min(3e4, Math.round(ms)));
    await sleep(clamped);
  ...
  async waitFor(options = {}) {
    await this.ensure();
    const text = typeof options.text === "string" && options.text.length > 0 ? options.text.slice(0, 2e3) : null;
    const selector = typeof options.selector === "string" && options.selector.length > 0 ? options.selector.slice(0, 1e3) : null;
    const urlIncludes = typeof options.urlIncludes === "string" && options.urlIncludes.length > 0 ? options.urlIncludes.slice(0, 2e3) : null;
    if (text === null && selector === null && urlIncludes === null) {
      return { ok: false, error: "provide at least one of text, selector, or urlIncludes" };
    }
    const timeoutMs = Math.max(0, Math.min(3e4, Math.round(options.timeoutMs ?? 1e4)));
    const pollMs = Math.max(50, Math.min(2e3, Math.round(options.pollMs ?? 250)));
  ...
  async assert(options = {}) {
    ...
    return {
      ...result,
      error: result.error?.startsWith("timed out waiting for") ? result.error.replace("timed out waiting for", "assertion failed:") : result.error
    };
  }
  async readUrl() {
    try {
  ...
  async back() {
    await this.ensure();
    const settling = this.waitForLoad(3e3);
    await this.cdp.call("Runtime.evaluate", { expression: "history.back()" }, 1e4);
    await settling;
    await sleep(300);
  ...
  async reload() {
    await this.ensure();
    const settling = this.waitForLoad(8e3);
    await this.cdp.call("Page.reload", {}, 15e3);
    await settling;
    await sleep(300);
    this.url = await this.readUrl();
    this.title = await this.readTitle();
  ...
  async click(target) {
    await this.ensure();
    const settling = this.waitForLoad(2500);
  ...
  async type(target, text) {
    await this.ensure();
    const resolve = typeof target === "number" ? `(window.__pilotEls && window.__pilotEls[${target} - 1] && window.__pilotEls[${target} - 1].el) || null` : `document.querySelector(${JSON.stringify(target)})`;
    const expression = `(() => {
        const el = ${resolve}
  ...
  async fill(fields) {
    ...
    for (const field of Array.isArray(fields) ? fields.slice(0, MAX_FILL_FIELDS) : []) {
      if (typeof field?.label !== "string" || field.label.trim().length === 0 || typeof field?.value !== "string") {
        ...
        requested.push({ label: field.label.trim().slice(0, 500), value: field.value.slice(0, 2e4) });
      }
    }
  ...
  async captureShot() {
    ...
    try {
      const shot = await this.cdp.call("Page.captureScreenshot", { format: "png" }, 15e3);
  ...
  async evalJs(expression) {
    ...
    if (text !== void 0 && text.length > MAX_EVAL) text = `${text.slice(0, MAX_EVAL)}\u2026(truncated)`;
  ...
  async stop() {
    ...
    if (child !== null) {
      ...
      for (let waited = 0; waited < 3e3 && child.exitCode === null && child.signalCode === null; waited += 50) {
        await sleep(50);
      }
    }
    if (this.profileDir !== null) await this.releaseProfile();
...
var PilotPool = class {
  ...
  panelSessions() {
    ...
    return [...this.pilots.entries()].sort((a, b) => b[1].lastUsedAt - a[1].lastUsedAt).map(([id, pilot]) => ({
      id,
      status: pilot.status,
      title: pilot.title,
      url: pilot.url,
  ...
  gc() {
    ...
    const candidates = [...this.pilots.entries()].filter(([key2]) => key2 !== this.primary && key2 !== this.panelSession).sort((a, b) => a[1].lastUsedAt - b[1].lastUsedAt);
...
function renderElements(elements, cap = 60) {
  const lines = (elements ?? []).slice(0, cap).map((el) => `[${el.ref}] <${el.tag}${el.type ? ` type="${el.type}"` : ""}> ${el.label || el.href || ""}`);
...
function diffSnapshots(prev, next) {
  ...
  const prevCounts = /* @__PURE__ */ new Map();
...
function renderChanged(changed) {
  if (changed === null || changed === void 0) return "no baseline yet (navigate first)";
  const parts = [
...
async function saveScreenshot(pilot, path) {
  await pilot.ensure();
...
${renderElements(value.elements, 40)}` }]
      ...
      define(
        ...
        (value) => [{ type: "text", text: `[${value.title}](${value.url})
${value.text.slice(0, 4e3)}${value.textLength > 4e3 ? `
...
${renderElements(value.elements)}` }]
      ...
      define(
        ...
        async (args, exec) => {
          ...
          if (target === void 0) throw new Error("provide ref or selector");
          return pilot.withOp(() => pilot.click(target));
        },
        (value) => [{ type: "text", text: value.ok ? `clicked ${value.ref !== void 0 ? `ref ${value.ref} ` : ""}<${value.tag}> "${value.text}" \u2014 title now: ${value.title}` : `click failed: ${value.error}` }]
      ),
      define(
        "pilot_type",
      ...
      define(
        ...
        (value) => [{ type: "text", text: value.ok ? `filled ${value.filled.length} field(s): ${value.filled.map((field) => field.label).join(", ")}` : `filled ${value.filled.length} field(s); ${value.failed.length} failed: ${value.failed.map((field) => `${field.label || "field"} (${field.error})`).join("; ")}` }]
      ),
      define(
        "pilot_upload",
      ...
      define(
        ...
        async (args, exec) => {
          const pilot = pool.for(exec.agent?.session?.id ?? "default");
          return pilot.withOp(() => pilot.wait(args.ms ?? 1e3));
        },
      ...
      define(
        ...
        async (args, exec) => {
          ...
          if (path === void 0) {
            let name2 = "";
            try {
              name2 = new URL(target).pathname.split("/").pop() ?? "";
            } catch {
            }
            if (name2 === "" || !/\.[A-Za-z0-9]{1,8}$/.test(name2)) name2 = `pilot-download-${Date.now()}`;
            const base = exec.agent?.session?.cwd ?? tmpdir();
            path = join(base, name2);
          }
